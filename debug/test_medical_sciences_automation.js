...
function getCurrentState() {
    ...
    // Find medical_sciences_singular mappings
    const medicalSciencesMappings = manualMapping.filter(mapping => {
        const original = mapping.originalName;
        const suggested = mapping.suggestedStandardizedName;
        return original.includes('Medical Sciences') && suggested === original.replace('Medical Sciences', 'Medical Science');
    });
    
    console.log(`\nFound ${medicalSciencesMappings.length} "Medical Sciences → Medical Science" mappings to automate:`);
    medicalSciencesMappings.forEach(mapping => {
        console.log(`  "${mapping.originalName}" -> "${mapping.suggestedStandardizedName}"`);
    });
    
    return {
        totalSchools,
        medicalSciencesMappings,
        originalManualMapping: manualMapping
    };
}

// Step 2: Add automation rule to canonicalizeName function
function addMedicalSciencesRule() {
    ...
    if (content.includes('Medical Sciences automation')) {
        console.log('Medical Sciences automation rule already exists!');
        return false;
    }
    ...
    const newRule = `    // Medical Sciences automation (e.g., "University of Medical Sciences" -> "University of Medical Science")
    ...
    console.log('Added Medical Sciences automation rule to canonicalizeName function');
    return true;
}

// Step 3: Comment out affected mappings
function commentOutMappings(medicalSciencesMappings) {
    console.log('\n=== STEP 3: TEMPORARILY COMMENTING OUT MAPPINGS ===');
    
    const manualMapping = JSON.parse(fs.readFileSync('frontend/public/data/manual-university-mapping.json', 'utf8'));
    const indicesToComment = medicalSciencesMappings.map(mapping => 
    ...
    // Remove the medical_sciences mappings temporarily
...
function finalizeChanges(success, originalTotalSchools, newTotalSchools, commentedIndices) {
    ...
    if (success && Math.abs(originalTotalSchools - newTotalSchools) <= 1) {
        ...
        const scrapeRankingsPath = 'scripts/scrape-rankings.js';
        let content = fs.readFileSync(scrapeRankingsPath, 'utf8');
        content = content.replace(/\s*\/\/ Medical Sciences automation.*\n.*cleaned = cleaned\.replace.*\n/, '');
...
function runFullTest() {
    console.log('🧪 COMPREHENSIVE MEDICAL SCIENCES AUTOMATION TEST\n');
    
    const baseline = getCurrentState();
    const ruleAdded = addMedicalSciencesRule();
    ...
    const commentedIndices = commentOutMappings(baseline.medicalSciencesMappings);
    ...
    console.log('\n=== FINAL RESULT ===');
    if (success) {
        console.log('🎉 Medical Sciences automation successfully implemented!');
        console.log(`✅ ${baseline.medicalSciencesMappings.length} mappings automated`);
